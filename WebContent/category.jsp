...
<%
  ...
  String type=request.getParameter("type");//brand
       ...
       Class.forName("oracle.jdbc.driver.OracleDriver");
		...
		if(type=="brand");//one of category
		{
			ps=con.prepareStatement("select distinct EAN from category where category=?");
...
<p><table width="992" height="179" border="1">
  ...
  <%while(rs.next())
    ...
    <% pid=rs.getString(1);
	ps=con.prepareStatement("select EAN,selling_price,title,amount from commodity where EAN=?");
		 ps.setString(1,pid);
		 rs1=ps.executeQuery();
		 while(rs1.next())
		 {
